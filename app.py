...
st.title("🛡️ GTM Upphandlingsbevakning & Säljinsikter (Claude)")
st.write("Automatisk sammanställning av offentliga upphandlingar med fokuserade säljanalyser via Claude.")

# Hämta API-nyckeln från Streamlit Secrets / miljövariabler
api_key = os.getenv("ANTHROPIC_API_KEY")

if not api_key:
    st.error("Ingen Anthropic API-nyckel hittades. Kontrollera att ANTHROPIC_API_KEY är inlagd under dina Secrets i Streamlit Cloud.")
    st.stop()

# Skapa klienten med nyckeln
...
if st.button("🚀 Hämta & Analysera med Claude", type="primary"):
    with st.spinner("Analyserar upphandlingar med Claude..."):
        all_tenders = fetch_tender_data()
        
        # Filtrering baserat på val i menyn
        ...
        for item in filtered_tenders:
            ...
            3. **GTM & Säljvinkel för konsultbolag:** 
               - Hur kan GTM-teamet agera på detta? (T.ex. kontakta myndigheten för tilläggstjänster/förändringsledning, eller kontakta den vinnande leverantören som underleverantör/partner).
            """
