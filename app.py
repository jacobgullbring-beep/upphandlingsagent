...
def scrape_all_pages(base_url):
    all_tenders = []
    page = 1
    max_pages = 20  # Säkerhetsspärr så loopen inte fastnar i oändlighet
...
if 'tender_df' in st.session_state and not st.session_state['tender_df'].empty:
    ...
    st.info(f"Visar {len(df_to_show)} av {len(st.session_state['tender_df'])} totala rader")
    st.dataframe(df_to_show, use_container_width=True)
    
    if st.button("🤖 Kör Sonnet 5-analys på det filtrerade urvalet"):
        with st.spinner("Analyserar med Sonnet 5..."):
            summary_data = df_to_show.head(30).to_string()
